...
//  file.  The key is generated by combining the lower bytes of the 
...
//                more kana to key.
//
static int test_nkey (byte *kana,byte *key) {
...
public:
  int   dlg_convert (HWND hwnd,UINT message,WPARAM wParam,LPARAM lParam);
...
private:
  int   check_error (int data,tchar *name);     // Check for import error.
...
struct godan {      // Structure relating ascii values for the godan doushi and their hiragana values.
  byte kana;        // kana value (just low byte).
  byte ascii;       // Ascii value of first character.
};
...
//
static int dialog_convert (HWND hwnd,UINT message,WPARAM wParam,LPARAM lParam) {
...
//
int EDIT_userconv::check_error (int data,tchar *name) {
...
//
int EDIT_userconv::dlg_convert (HWND hwnd,UINT message,WPARAM wParam,LPARAM lParam) {
  KANJI *kkana,*kkanji;
  int    lkana, lkanji;
  int i;
  switch (message) {
...
//
void EDIT_userconv::put_data (byte *data,tchar *name) {
  int j;
...
int JWP_conv::save_user () {
  ...
  WriteFile   (file,user,strlen((char *) user),&done,NULL);
  CloseHandle (file);
  return      (false);
}
...
//
int JWP_conv::binary_search (byte *key) {
...
//
int JWP_conv::index_compare (byte *key,int id) {
  int i;
...
//  
  user = load_image(jwp_config.name(NAME_USERCONV,OPEN_READ,true));
//
//  Generate window.
//
  if (wclass) {
...
//
void JWP_conv::realloc (int size) {
  KANJI_sel *new_user;
  int    i;
...
//                 etc.).
//
void JWP_conv::search (byte *key,int endkana) {
  int i;
  IO_cache cache;
#if   (CONVERT_ACCESS == CONVERT_ACCESS_MEMORY)
...
#else  CONVERT_ACCESS
  byte buffer[SIZE_BUFFER];         // Size of buffer used to read the WNN kana->
                                    // kanji dictionary.  This must read multable
                                    // lines.  The longest lines are above 256 bytes.
  i = binary_search(key);           // Find key in index?
  if (i >= 0) {
...
//
void JWP_conv::search_dict (byte *key,int endkana,IO_cache *cache,int sorted) {
  byte *kana;               // Points to kana string from ditionary (in key form).
  char  dict_ending;        // Pointer to ending code from dictonary.
  byte *convert;            // Convert string from dictionary (kanji/kanji/...)
  char ending = '*';        // Type of ending allowed for this conversion.
  TCHAR *ascii;              // Points to ascii representaiton for endkana.
  byte *ptr;
  int   i,j,diff;
...
//
  for (ptr = cache->get_line(); ptr && *ptr; ptr = cache->get_line()) {
    ...
    while (*ptr && (*ptr > 0x7f)) ptr++;        // Find first non-key charcter
    dict_ending = *ptr++;
