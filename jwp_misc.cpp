...
int get_float (HWND hwnd,int id,float min_val,float max_val,float def,int scale,float *address) {
  ...
  if (1 != swscanf(buffer,TEXT("%g"),&value)) value = def;
    else {
...
TCHAR *get_folder (int id,TCHAR *buffer) {
  ...
  switch (id) {
    case CSIDL_DESKTOP:
         lstrcpyW (buffer,TEXT("\\windows\\desktop"));
         break;
    case CSIDL_PROGRAMS:
         lstrcpyW (buffer,TEXT("\\windows\\start menu\\programs"));
         break;
    default:
    case CSIDL_PERSONAL:
         lstrcpyW (buffer,TEXT("\\my documents"));
         break;
  }
  return (buffer);
...
//
KANJI *k_wcsdup (KANJI *string,int length) {
  KANJI *dup;
...
int JMessageBox (HWND hwnd,int text,int caption,UINT type,...) {
  ...
  wvsprintfW  (buffer,message,argptr);
...
void put_float (HWND hwnd,int id,float value,int scale) {
  TCHAR buffer[100];
  swprintf (buffer,TEXT("%g"),value/scale);
...
void RecentList::insert (HMENU menu,int pos,TCHAR *text) {
  TCHAR buffer[SIZE_BUFFER];
  int   i;
  for (i = RECENT_MAX-2; i >= pos; i--) lstrcpyW (list[i+1],list[i]);
  lstrcpyW    (list[pos],text);
  if (lstrlen(text) <= RECENT_LEN) lstrcpyW (buffer,text);
    else {
      buffer[0] = text[0];
      buffer[1] = text[1];
      lstrcpyW (buffer+2,TEXT(" ..."));
...
void RecentList::remove (HMENU menu,int pos) {
  int i;
  for (i = pos; i <= RECENT_MAX-2; i++) lstrcpyW (list[i],list[i+1]);
...
long get_menudata (HMENU menu,int item,int position,TCHAR *buffer) {
  ...
  lstrcpyW (buffer,recent_list.get(item));
  return (info.wID);
}
