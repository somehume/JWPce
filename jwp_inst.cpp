...
static void save_link (IPersistFile *file,int place,tchar *dir) {
  ...
  MultiByteToWideChar (CP_ACP,0,buffer,-1, (LPWSTR) wbuffer,SIZE_BUFFER);
  file->Save ((LPWSTR) wbuffer,true);
#endif UNICODE
  return;
}
#endif WINCE
...
//
//      subkey -- Key location.
//      format -- Format for key value, followed by swprintf parameters.
//
...
static void make_key (tchar *subkey,tchar *format,...) {
  ...
  wvsprintfW (value ,format,argptr);
...
//
    ...
    if (S_OK == CoCreateInstance(CLSID_ShellLink,NULL,CLSCTX_INPROC_SERVER,IID_IShellLink,(void **) &link)) {
      ...
      link->SetDescription (_T("JWPce"));           // This is what will show up in the start-menu.
