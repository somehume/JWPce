...
//
             ...
             lstrcpyW (lf.lfFaceName,jwp_config.cfg.ascii_font.name);
...
static TCHAR *format_fc (TCHAR *buffer,int main,int index) {
  if (main == -1) buffer[0] = 0; else wsprintfW (buffer,TEXT("%04d.%d"),main,index);
  return (buffer);
}
...
static TCHAR *format_int (TCHAR *buffer,int value) {
  buffer[0] = 0;
  if (value) wsprintfW (buffer,TEXT("%d"),value);
  return   (buffer);
}
...
static void format_two (HWND hwnd,int id,TCHAR *buffer1,TCHAR *buffer2) {
  TCHAR buffer[80];
  wsprintfW       (buffer,TEXT("%s\t%s"),buffer1,buffer2);
...
#else WINCE
  #define info_string(w,i,s) SetDlgItemText(w,i,(TCHAR *) s);
#endif WINCE
...
//
  ...
  if (code && (code <= INFO_MAXITEM)) wsprintfW (buffer1,TEXT("%s:"),get_string(info_codes[code])); else buffer1[0] = 0;  
...
//  if (!kinfo.extra && (code >= INFO_SPAHN)) return;
  switch (code) {
    ...
    case INFO_JIS:
         ...
         wsprintfW       (buffer1,TEXT("%X (%X) [%s]"),ch,ch | 0x8080,buffer2);
         SetDlgItemText (hwnd,line,buffer1);
         break;
    case INFO_SHIFTJIS:
         wsprintfW       (buffer1,TEXT("%X"),(ch < 0x80) ? ch : jis2sjis(ch));
         SetDlgItemText (hwnd,line,buffer1);
         break;
    case INFO_UNICODE:
         wsprintfW       (buffer1,TEXT("%X"),jis2unicode(ch));
         SetDlgItemText (hwnd,line,buffer1);
         break;
    case INFO_STROKE:
    ...
    case INFO_NELSON:
         ...
         break;
    case INFO_HALPERN:
         if (kinfo.skip_t) wsprintfW (buffer2,TEXT("%d-%d-%d"),kinfo.skip_t,kinfo.skip_1,kinfo.skip_2); else buffer2[0] = 0;
         format_two (hwnd,line,format_int(buffer1,kinfo.halpern),buffer2);
         break;
    case INFO_SPAHN:
         if (extend.sh_rstroke || extend.sh_ostroke) wsprintfW (buffer1,TEXT("%d%c%d.%d"),extend.sh_rstroke,extend.sh_radical+'a',extend.sh_ostroke,extend.sh_index); else buffer1[0] = 0;
    ...
    case INFO_MOROHASHI:
         if (!extend.md_long) buffer1[0] = 0;
             ...
             if      (extend.md_x) wsprintfW (buffer1,TEXT("%dX"),extend.md_long);
             else if (extend.md_p) wsprintfW (buffer1,TEXT("%dP"),extend.md_long);
             else                  wsprintfW (buffer1,TEXT("%d") ,extend.md_long);
           }
         if (extend.md_short1) wsprintfW (buffer2,TEXT("%d.%04d"),extend.md_short1,extend.md_short2); else buffer2[0] = 0;
...
//        
    ...
    if (!kinfo.classical) SetDlgItemInt (hwnd,IDC_KIBUSHU,kinfo.bushu,true);
        ...
        wsprintfW (buffer2,TEXT("%d (%d)"),kinfo.bushu,kinfo.classical);
...
//
                  ...
                  for (i = 0; i < SIZE_KANJI; i++) {
                    ...
                    if (frequency) {                            // Put frequncy
                      line.put_char (KANJI_SPACE);
                      wsprintfW (text,TEXT("%d"),kanji_count->list[i].count);
