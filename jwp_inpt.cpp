...
//
TCHAR direct_kana[SIZE_DIRECT][4] = {
  _T("+a") ,_T("a") ,_T("+i"),_T("i") ,_T("+u") ,_T("u") ,_T("+e") ,_T("e")  ,_T("+o") ,_T("o") ,
  _T("ka") ,_T("ga"),_T("ki"),_T("gi"),_T("ku") ,_T("gu"),_T("ke") ,_T("ge") ,_T("ko") ,_T("go"),
  _T("sa") ,_T("za"),_T("si"),_T("zi"),_T("su") ,_T("zu"),_T("se") ,_T("ze") ,_T("so") ,_T("zo"),
  _T("ta") ,_T("da"),_T("ti"),_T("di"),_T("+tu"),_T("tu"),_T("du") ,_T("te") ,_T("de") ,_T("to"),
  _T("do") ,_T("na"),_T("ni"),_T("nu"),_T("ne") ,_T("no"),_T("ha") ,_T("ba") ,_T("pa") ,_T("hi"),
  _T("bi") ,_T("pi"),_T("hu"),_T("bu"),_T("pu") ,_T("he"),_T("be") ,_T("pe") ,_T("ho") ,_T("bo"),
  _T("po") ,_T("ma"),_T("mi"),_T("mu"),_T("me") ,_T("mo"),_T("+ya"),_T("ya") ,_T("+yu"),_T("yu"),
  _T("+yo"),_T("yo"),_T("ra"),_T("ri"),_T("ru") ,_T("re"),_T("ro") ,_T("+wa"),_T("wa") ,_T("wi"),
  _T("we") ,_T("wo"),_T("n'"),
};
...
//
static int find_direct (TCHAR *string)
{
  int i;
  for (i = 0; i < SIZE_DIRECT; i++) if (!wcscmp(string,direct_kana[i])) return (i+0x21);
  return (0);
}
...
typedef struct kana_state {
  ...
  byte length;      // Number of diffent state lines for this groupt.  (Only valid on first line of state group.)
  byte next;        // Next state.  KANA_DONE indicates we are at the end.
} kana_state;
...
//
  ...
  if ((index == 1) && !pending && (((buffer[0] == ch) && isalpha(ch)) || ((buffer[0] == 't') && (ch == 'c')))) {
    if (c == ch) wcscpy (buffer, _T("+tu")); else wcscpy (buffer, _T("+TU"));
...
//
  for (i = 0; compound_kana[i].kana[0]; i++) {
    if (!wcscmp(compound_kana[i].string,buffer)) {
...
//
  ...
  static struct complex_kana {
    ...
    { _T("+ka"), { 0x2575,0x0    } },
    { _T("+ke"), { 0x2576,0x0    } },
    { _T("`")  , { 0x2156,0x0    } },   // These two are necessary because they 
    { _T("'")  , { 0x2157,0x0    } },   // translate to non-ASCII equivalents   
    { _T("y=") , { 0x216f,0x0    } },   // These are special additions (yen)
    { _T("f-") , { 0x2172,0x0    } },   //   (lira)
    { _T("^^") , { 0x2130,0x0    } },   //   (^)
    { _T("^.") , { 0x2126,0x0    } },   //   (center .)
    { _T("^-") , { 0x2144,0x0    } },   //   (...)
    { _T("^+") , { 0x215c,0x0    } },   //   (+)
  };
  for (i = 0; i < SIZE_COMPLEX; i++) {
    if (!wcscmp(complex_kana[i].string,buffer)) {
...
//
  static TCHAR temp[4] = _T("xi");           // temp is used to search for first part of compount kana like kya,gyo, etc.
  temp[0]   = buffer[0];                // first kara is first char (n)+(i).
  buffer[0] = _T('+');                      // second kana is a small verion.
