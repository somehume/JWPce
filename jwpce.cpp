...
#ifdef UNICODE
#define NAME_CONFIG TEXT("jwpceu.cfg")
#else
...
#endif WINCE
  lstrcpyW (ptr,file);                               // Single user/system file names
  if (!net || !nptr) return (last_name = buffer);   // Not user file or not netwrok running.
  lstrcpyW (nptr,file);                              // Network located file.    
...
static void check_height (HWND hwnd) {
  ...
  EnableWindow (GetDlgItem(hwnd,IDC_OFHEIGHT),(FONT_PRINT == i) || (lstrlen(buffer) >= 4) && _wcsicmp(buffer+lstrlen(buffer)-4,TEXT(".f00")) && ((FONT_BIG != i) && (FONT_JIS != i) && !IsDlgButtonChecked(hwnd,IDC_OFAUTO)));
  return;
}
...
//
  ...
  if (!(language = LoadLibrary(jwp_config.name(TEXT("JWPce_lang.dll"),OPEN_READ,false)))) language = instance;
      ...
      if ((ptr[0] != LANGUAGE_CODE) || _wcsnicmp(ptr+1,VERSION_STRING,4)) {
...
//
  ...
  if (tip_data = (TCHAR *) malloc(sizeof(TCHAR)*(1+lstrlen(ptr)))) {    // Allocate storage.  If not possible we just forget the tips.
    lstrcpyW (tip_data,ptr);                                             // Copy tips to the data array.
