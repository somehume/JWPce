...
#ifdef UNICODE
#define NAME_DICTIONARIES   TEXT("jwpceu.dic")
#else
...
static TCHAR *index_name (TCHAR *name,TCHAR *buffer) {
  int i;
  if (!buffer) buffer = name; else lstrcpyW (buffer,name);
...
static TCHAR *relative_name (TCHAR *buffer) {
  ...
  if (_wcsnicmp(buffer,ptr,lstrlen(ptr))) return (buffer);
...
//
           ...
           case IDC_DSBROWSE:
                ...
                dic = get_dictionary(hwnd);
                if (!dic->name[0]) buffer[0] = 0; else lstrcpyW (buffer,dic->get_name());
                delete dic;
...
static void check_buttons (HWND hwnd) {
  ...
  EnableWindow (GetDlgItem(hwnd,IDC_EDITLISTDELETE),lstrcmpW(buffer,get_string(IDS_DC_USER)));
  return;
}
...
static Dictionary *get_dict (HWND list,int index) {
  ...
  if      (!lstrcmpW(buf1,get_string(IDS_DC_FORMATEUC ))) *ptr++ = DICTFLAG_EUC;
  else if (!lstrcmpW(buf1,get_string(IDS_DC_FORMATUTF8))) *ptr++ = DICTFLAG_UTF8;
  else                                                   *ptr++ = DICTFLAG_MIXED;
  ListView_GetItemText (list,index,COLUMN_NAMES,buf1,SIZE_BUFFER);
  if      (!lstrcmpW(buf1,get_string(IDS_DC_NAMES     ))) *ptr++ = DICTFLAG_NAMES;
  else if (!lstrcmpW(buf1,get_string(IDS_DC_NAMESONLY ))) *ptr++ = DICTFLAG_NAMESONLY;
  ListView_GetItemText (list,index,COLUMN_SPECIAL,buf1,SIZE_BUFFER);
  if      (!lstrcmpW(buf1,get_string(IDS_DC_USER      ))) *ptr++ = DICTFLAG_USER;
  else if (!lstrcmpW(buf1,get_string(IDS_DC_CLASSICAL ))) *ptr++ = DICTFLAG_CLASSICAL;
  ListView_GetItemText (list,index,COLUMN_TYPE,buf1,SIZE_BUFFER);
  if      (!lstrcmpW(buf1,get_string(IDS_DC_TYPEIDX   ))) *ptr++ = DICTFLAG_INDEX;
  else if (!lstrcmpW(buf1,get_string(IDS_DC_TYPEBUF   ))) *ptr++ = DICTFLAG_BUFFER;
  ListView_GetItemText (list,index,COLUMN_KEEP,buf1,SIZE_BUFFER);
  if      (!lstrcmpW(buf1,get_string(IDS_DC_YES       ))) *ptr++ = DICTFLAG_KEEP;
  ListView_GetItemText (list,index,COLUMN_QUIET,buf1,SIZE_BUFFER);
  if      (!lstrcmpW(buf1,get_string(IDS_DC_YES       ))) *ptr++ = DICTFLAG_QUIET;
  *ptr = 0;
...
DeleteItem:;
                ...
                if (!lstrcmpW(buffer,get_string(IDS_DC_USER))) return (0);         // Don't allow the user dictionary to be deleted.
...
void JWP_dict::error (int format,...) {
  ...
  wvsprintfW  (buffer,string,argptr);
...
void JWP_dict::message (tchar *format,...) {
  ...
  va_list argptr;
  va_start (argptr,format);
  if (format) wvsprintfW (buffer,format,argptr);
...
void JWP_dict::search_dict () {
  ...
  TCHAR  *ptr;                       // Holds romaji for kana character at end of search string.
//
...
// TODO -- This needs to be added.    
    case DICT_MEMORY: 
         switch (format) {
           default:
		   case DICT_EUC:   dic = EUC_MEM_Dictionary::CreateDict(); break;//new EUC_MEM_Dictionary; break;
		   case DICT_MIXED: dic = MIX_MEM_Dictionary::CreateDict(); break;//new MIX_MEM_Dictionary; break;
		   case DICT_UTF8:  dic = UTF_MEM_Dictionary::CreateDict(); break;//new UTF_MEM_Dictionary; break;   
         }
         break;
    case DICT_INDEX:  
         switch (format) {
           default:
		   case DICT_EUC:   dic = EUC_IDX_Dictionary::CreateDict(); break;//new EUC_IDX_Dictionary; break;
		   case DICT_MIXED: dic = MIX_IDX_Dictionary::CreateDict(); break;//new MIX_IDX_Dictionary; break;
		   case DICT_UTF8:  dic = UTF_IDX_Dictionary::CreateDict(); break;//new UTF_IDX_Dictionary; break;   
         }
         break;
  }
